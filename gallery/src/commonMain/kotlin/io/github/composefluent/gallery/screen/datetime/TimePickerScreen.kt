...
private fun TimePickerMinuteIncrementSample() {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ComboBox(
            header = "Minute increment",
            items = minuteIncrements.map { it.toString() },
            selected = minuteIncrements.indexOf(minuteIncrement),
            onSelectionChange = { _, increment -> minuteIncrement = increment.toInt() }
        )
        TimePicker(
            value = value,
            onValueChange = { value = it },
